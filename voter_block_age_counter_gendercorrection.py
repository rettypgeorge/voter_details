...
for pdf in pdfs:
    ...
    for page_no in range(START_PAGE - 1, len(pages)):
        ...
        for x, y, w, h in blocks:
            ...
            # ---------- GENDER (FIXED & GUARANTEED) ----------
            # Crop RIGHT-side gender box
            gx1, gx2 = int(w * 0.65), int(w * 0.95)
            gy1, gy2 = int(h * 0.15), int(h * 0.85)

            gender_crop = block[gy1:gy2, gx1:gx2]
            gender_crop = cv2.threshold(gender_crop, 150, 255, cv2.THRESH_BINARY)[1]

            gender_text = pytesseract.image_to_string(
                gender_crop, lang="mal", config="--psm 6"
            )

            if "ആൺ" in gender_text:
                gender = "Male"
            else:
                gender = "Female"   # DEFAULT RULE
